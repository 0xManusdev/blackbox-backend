...
export async function analyzeAndAnonymize(content) {
  ...
  "analysis": "Analyse professionnelle ici"
    ...
    try {
        ...
        if (!parsed.anonymizedContent || !parsed.category || !parsed.severity || !parsed.analysis) {
            throw new AppError('Incomplete AI analysis', 500);
        }
        return parsed;
    }
    catch (error) {
        ...
        throw new AppError('Failed to analyze incident with AI', 500);
    }
}
